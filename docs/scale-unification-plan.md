...
| 5 | ◑ **5a shipped** — streamed sector/region particles keyed off the float64 frame (decision 3); retired to a default-off toggle after live QA (read as a hard square), **star-shells** now carry the neighbourhood→galaxy visual, catalog gets an aggressive zoom-exterior LOD. **Remaining:** near-particle → volumetric-cloud luminance handoff; ±20–25% per-sector density; sectors as navigable traversal overlays. **The stellar-phenomena layer** (nebulae/remnants/exotica) that shares this sector placement is a dedicated effort — see [`stellar-phenomena-plan.md`](stellar-phenomena-plan.md). | medium | new far systems |
